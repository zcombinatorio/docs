...
Creates an unsigned transaction for claiming accumulated fees from a Meteora DAMM v2 (Dynamic AMM) pool. The API validates available fees, builds a transaction that claims fees and transfers them to the configured destination address, and returns the transaction ready for user signing.
...
Combinator takes a **0.5% protocol fee** on trading volume. This fee accumulates in the pool and can be claimed via this endpoint. LP fees are separate and configurable by the DAO creator.
...
3. The transfer uses `SystemProgram.transfer` instead of SPL token transfer
