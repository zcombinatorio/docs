...
Deposit (split) regular tokens into conditional tokens to participate in decision markets. When you deposit, your tokens are split equally into conditional tokens for each outcome option.

```
1 regular token → 1 conditional token per outcome
```

For example, depositing 100 base tokens into a proposal with 2 outcomes gives you 100 conditional tokens for each outcome.
...
| Type | Token | Use Case |
|------|-------|----------|
| `base` | DAO's native token | Trading the base asset |
...
// 1. Deposit 100 base tokens to get 100 tokens for each outcome
...
// 3. If outcome 0 wins, redeem for ~200 base tokens
