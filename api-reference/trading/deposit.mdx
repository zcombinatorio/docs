...
Deposit (split) regular tokens into conditional tokens to participate in Quantum Markets. When you deposit, your tokens are split equally into pass and fail conditional tokens.

```
1 regular token → 1 pass token + 1 fail token
```

For example, depositing 100 ZC gives you 100 pass-ZC and 100 fail-ZC.
...
```javascript
// 1. Deposit 100 ZC to get 100 pass-ZC + 100 fail-ZC
await depositTokens(proposalPda, wallet, 'base', '100000000000', signTransaction);

// 2. Swap fail-ZC for pass-ZC in the fail market
// Now you have ~200 pass-ZC and 0 fail-ZC

// 3. If proposal passes, redeem pass-ZC for ~200 ZC
// Net profit if pass price was < 0.5
```
