...
description: "Programmatic access to decision markets on Combinator"
---

## Overview

The Trading API enables developers to build applications that interact with decision markets on Combinator. All trading operations use a **build/execute pattern** where transactions are constructed server-side and signed client-side.
...
- **`base`** - The DAO's native token
- **`quote`** - The quote currency (e.g., USDC)

## Pool Indices

Each proposal has one pool per outcome option. The number of pools depends on how many options the proposal creator defined.
...
Use the [Market Status](/api-reference/trading/market-status) endpoint to see how many options a proposal has (`numOptions`).
