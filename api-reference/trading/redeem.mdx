...
|-----------------|------------------|-------|
| Outcome n | Outcome-n tokens | 1:1 for regular |
...
<ResponseField name="winningIndex" type="number">
  The index of the winning outcome (corresponds to outcome option)
</ResponseField>
...
async function redeemWinnings(proposalPda, wallet, vaultType, signTransaction) {
  ...
  console.log(`Redeeming tokens. Winning outcome index: ${winningIndex}`);
...
These examples assume a proposal with 2 outcome options.

**Scenario 1: You held outcome-0 tokens, outcome 0 won**
```
Before: 100 outcome-0-BASE, 0 outcome-1-BASE
Redeem: 100 outcome-0-BASE -> 100 BASE
After: 100 BASE
```

**Scenario 2: You held outcome-0 tokens, outcome 1 won**
```
Before: 100 outcome-0-BASE, 0 outcome-1-BASE
Result: Outcome-0 tokens are worthless
After: 0 BASE (loss)
```

**Scenario 3: You held equal positions (hedged)**
```
Before: 50 outcome-0-BASE, 50 outcome-1-BASE
Outcome 0 wins: Redeem 50 outcome-0-BASE -> 50 BASE
Lose: 50 outcome-1-BASE (worthless)
After: 50 BASE
```
...
```javascript
async function decisionMarketsTradingExample() {
