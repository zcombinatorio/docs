...
```
1 token of each outcome → 1 regular token
```

You must have **equal amounts** of all conditional tokens to withdraw. This is the inverse of depositing.
...
- **Exit without position** - If you have equal amounts of each conditional token, merge them back
...
```json
// This position CANNOT be withdrawn directly (example with 2 outcomes):
{
  "conditionalBalances": ["150000000", "50000000"]  // 150 outcome-0, 50 outcome-1
}

// First swap to equalize your balances
// Then you can withdraw 50 (the minimum across all outcomes)
```
...
- After withdrawal, equal amounts of each conditional token are burned
